...
var botClient = new TelegramBotClient("6581805118:AAE6xf4-hozhVgGun9JPnUb0dPmRTro-R34");
...
async Task HandleTextMessageAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken, string user) 
{
    ...
    var courses = JsonConvert.DeserializeObject<List<Valuta>>(body);
    ...
    foreach (Valuta item in courses)
    {
        if (button.Count != 6)
        {
...
public class Valuta
{
    ...
    public decimal cb_price { get; set; }
